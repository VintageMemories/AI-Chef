...
3. **多轮对话与流式输出**
   - 基于 LangGraph 的多轮对话，支持 SSE（Server-Sent Events）流式输出和 Markdown 渲染。
   - 打字机效果逐字显示，体验流畅自然。

4. **会话记忆**
   - 基于 SQLite + SqliteSaver 的对话持久化，支持历史消息查询与清空。
   - 同一 `thread_id` 下的多轮对话自动关联上下文。

5. **OSS 直传**
   - 图片通过预签名 URL 直传阿里云 OSS，不占用服务器带宽。
   - 前端先获取上传授权，再直传文件，后端仅负责签名生成。

6. **现代化前端**
   - 响应式聊天界面，支持图片预览、打字动画、移动端适配。
...
│   ├── db/                             # SQLite 数据库目录（运行时自动生成 ai_chef.db）
│   └── static/
│       └── index.html                  # 前端聊天界面
...
3. **配置环境变量**
   ...
   > ⚠️ **注意**：OSS 凭证环境变量名必须为 `ALIBABA_CLOUD_ACCESS_KEY_ID` 和 `ALIBABA_CLOUD_ACCESS_KEY_SECRET`，这是阿里云 SDK 自动读取的标准变量名，不要写成其他名称。
...
- 阿里云百炼 DashScope：[https://dashscope.aliyun.com/](https://dashscope.aliyun.com/)
...
- 阿里云 OSS 文档：[https://help.aliyun.com/product/31815.html](https://help.aliyun.com/product/31815.html)
